...
const typeOfPaces = [
  ...
  {
    name: "Private area by portion",
    description:
      "Same property for multiple people, each with their own partition",
  },
  {
    name: "Hotel room",
    description:
...
const TabFilters = () => {
  ...
  const [bedrooms, setBedrooms] = useState<number[]>([3, 3, 2]);
  const [extraFilters, setExtraFilters] = useState<object>({});
  // const [tempChanges, setTempChanges] = useState<tempChangeState>();

  const [tempPlaceType, setTempPlaceType] = useState<string>("");
                    ...
                    {typeOfPaces.map((item) => (
                      <div key={item.name} className=" flex">
                        ...
                        <input
                          type="checkbox"
                          id="inputCheckBox"
                          className=" rounded-md p-3 mr-4 bg-gray-600 cursor-pointer"
                          // onClick={() => setPlaceType(item.name)}
                          // onClick={() => {
                          //   const newTempState = {...tempChanges};
                          //   if (newTempState.includes(item.name)) {
                          //     newTempState.splice(
                          //       newTempState.indexOf(item.name),
                          //       1
                          //     );
                          //   } else {
                          //     newTempState.push(item.name);
                          //   }
                          //   setTempChanges(newTempState);
                          // }}
                          onClick={() => setTempPlaceType(item.name)}
                        />
                        <div>
                          <label
